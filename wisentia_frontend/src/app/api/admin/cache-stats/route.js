...
export async function GET(request) {
  ...
  try {
    ...
    console.log('Cache Stats API: Token bilgisi:', token ? `${token.substring(0, 15)}...` : 'Yok');
  } catch (error) {
  ...
  try {
    ...
    const backendUrl = 'http://localhost:8000/api/admin/cache-stats/';
    ...
    console.error('Cache Stats API Proxy hatası:', error);
    return NextResponse.json(
