...
export async function POST(request) {
  ...
  try {
    ...
    console.log('Cache Clear API: Token bilgisi:', token ? `${token.substring(0, 15)}...` : 'Yok');
  } catch (error) {
  ...
  try {
    ...
    const backendUrl = 'http://localhost:8000/api/admin/cache-clear/';
    ...
    const response = await fetch(backendUrl, {
      ...
      body: JSON.stringify({}) // POST isteği için boş body
    });
    ...
    console.error('Cache Clear API Proxy hatası:', error);
    return NextResponse.json(
