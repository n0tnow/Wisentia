...
export async function GET(request) {
  ...
  try {
    ...
    console.log('Community API Proxy: Token bilgisi:', token ? `${token.substring(0, 15)}...` : 'Yok');
  } catch (error) {
  ...
  try {
    ...
    const backendUrl = `http://localhost:8000/api/community/posts/?${queryParams.toString()}`;
...
// Post durumunu değiştirmek için (admin panel özel fonksiyonu)
export async function PUT(request) {
  ...
  try {
    ...
    // Backend API URL - Django API'sine istek yapıyoruz
    const backendUrl = `http://localhost:8000/api/admin/community/posts/${postId}/status/`;
    console.log('Backend API isteği yapılıyor:', backendUrl);
...
export async function DELETE(request) {
  ...
  try {
    // Backend API URL
    const backendUrl = `http://localhost:8000/api/admin/community/posts/${postId}/`;
    console.log('Backend API isteği yapılıyor:', backendUrl);
